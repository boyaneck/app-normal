...
const db = {
  ban: {
    upsert: async (options) => {
      console.log("DB Upsert (Ban):", options);
      return { id: "mock-ban-id", ...options.create };
    },
  },
};
// -----------------------------------------
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!streamer) {
      return res
        .status(403)
        .json({ message: "Forbidden: Only the streamer can moderate." });
    }
    ...
    switch (data.action) {
      case "mute":
        ...
        console.log(
          `Attempting to mute ${data.targetIdentity} in ${
            data.roomName
      ...
      case "kick":
        ...
        banRecordData = {
          where: {
            roomName_userIdentity: {
              roomName: data.roomName,
              userIdentity: data.targetIdentity,
            },
          },
          update: {
            banType: "kick",
            expiresAt: kickExpiresAt,
            bannedBy: streamer.identity,
            reason: data.reason,
          },
          create: {
            roomName: data.roomName,
        ...
        console.log(
          `Attempting to kick ${data.targetIdentity} from ${data.roomName} for ${data.durationMinutes} minutes`
        );
        break;
      ...
      case "ban":
        ...
        console.log(
          `Attempting to permanently ban ${data.targetIdentity} from ${data.roomName}`
        );
        break;
      ...
      default:
        return res
          .status(400)
          .json({ message: "Bad Request: Invalid action type." });
    }
    ...
    if (kickUser) {
      ...
      try {
        ...
        console.log(
          `Participant ${data.targetIdentity} removed from ${data.roomName}`
        );
      } catch (error) {
        ...
        if (error.code !== 404) {
          ...
          console.log(
            `Participant ${data.targetIdentity} was likely already gone.`
          );
        }
      }
    }
